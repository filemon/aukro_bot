...
module Aukro
  ...
  class Bidder
    ...
    def get_actual_price (auction)
      ...
      if page =~ %r{<strong id="priceValue" class="price">(.*?)( K.*)</strong>}m
        $1.gsub(/[^0-9]/, '').to_i
      end
    end
    ...
    def increase_price(auction, price)
      begin
        response = @soap_client.call(:do_bid_item,
                                     :message => {
                                         :'sessionHandle' => @session[:session_handle_part],
                                         :'bidItId' => auction.id,
                                         :'bidUserPrice' => price,
                                         :'bidQuantity' => 1,
                                         :'bidBuyNow' => 0})
    ...
    def login
      response = @soap_client.call(:do_login_enc,
                                   :message => {
                                       :'userLogin' => @aukro_config['username'],
                                       :'userHashPassword' => Utils.encode_pass(@aukro_config['password']),
                                       :'countryCode' => @aukro_config['aukro_country_code'],
                                       :'webapiKey' => @aukro_config['web_api_key'],
                                       :'localVersion' => @aukro_config['api_local_version']})
